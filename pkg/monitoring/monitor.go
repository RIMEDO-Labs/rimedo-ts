...
func NewMonitor(streamReader broker.StreamReader, nodeID topoapi.ID, metricStore store.Store, nodeManager *NodeManager, triggerType *e2sm_mho.MhoTriggerType, flag bool) *Monitor {
	return &Monitor{
...
func (m *Monitor) processIndication(ctx context.Context, indication e2api.Indication, nodeID topoapi.ID) error {
	...
	if m.smRcFlag {
		...
		m.nodeManager.SendThroughInformationChannel(message)
	}
